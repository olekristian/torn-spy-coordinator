...
Employees should use a narrowly scoped Custom key and must not use a Full Access key. Sign-in requires `user → profile`; company membership is verified from the authenticated `key → info` response. Optional report lookup additionally requires `user → reports`. Signing out removes the key and employee session from that browser session.
